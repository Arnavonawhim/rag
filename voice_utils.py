...
try:
    ...
    try:
        ...
        # This will catch the "No such audio device" error on the server
...
def create_voice_input_interface():
    ...
    if st.session_state.is_recording:
        duration = st.session_state.get('voice_duration', 10)
    ...
    # Return any previously recognized text that's ready to use
    if st.session_state.voice_input_ready and st.session_state.voice_text:
    ...
    with tab2:
        ...
        with col2:
            ...
            if PYGAME_AVAILABLE:
                ...
                st.info("Pygame not available - using Streamlit player only")
...
pip install pydub simpleaudio
        ...
        if st.button("Clean Temporary Audio Files"):
            temp_dir = os.path.join(os.getcwd(), "temp_audio")
